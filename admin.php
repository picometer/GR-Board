...
?>
		...
		<div class="mvBack" id="admLatestArticle">
			...
			<div class="halfBox">
			<strong title="아래에 GR보드에서 작성된 최신 글들이 통합되어 출력됩니다.">&middot; 통합 최근 게시물</strong>
				<ul>
					<?php
					$getTA = $GR->query('select * from '.$dbFIX.'total_article order by no desc limit 10');
			...
			<div class="halfBox">
			<strong title="아래에 GR보드에서 작성된 최신 댓글(코멘트)들이 통합되어 출력됩니다.">&middot; 통합 최근 댓글</strong>
				<ul>
					<?php
					$getTC = $GR->query('select * from '.$dbFIX.'total_comment order by no desc limit 10');
		...
		<div class="mvBack" id="admLoginLogList">
			...
			$loginlogFromRecord = ($loginlogPage - 1) * 20;
			$totalloginlogNum = $GR->getArray('select count(*) from '.$dbFIX.'login_log');
			$loginlogTotalPage = ceil($totalloginlogNum[0] / 20);

			// 최근 로그인한 기록들 열람
			$getLoginLog = $GR->query('select * from '.$dbFIX.'login_log order by no desc limit '.$loginlogFromRecord.', 20');
			$loopLogin = $loginlogFromRecord+1;
			$chkLoop = 0;
			while($loginlog = $GR->fetch($getLoginLog))
			{
				if($chkLoop && (($loopLogin-1) % 10 == 0)) echo '</div><div class="halfBox">';
				...
				echo '<div style="padding: 3px; border-bottom: #eee 1px solid" title="IP: '.$loginlog['ip'].'">'.$loopLogin.'. '.$name.' <span class="caution">(level: '.$m['level'].' / point: '.$m['point'].' / time: '.date('m.d H:i:s', $loginlog['signdate']).')</span></div>'; 
				$loopLogin++;
				$chkLoop++;
			}
