...
$setting['thumb_width_size'] = 180; // 썸네일 폭 지정 (px 단위)
// 갤러리 설정값 조정하기 끝
...
// 썸네일 이미지 만들기
function makeThumbImage($path, $size)
{
	global $theme;
...
function setArticleData($list, $size)
{
	...
	$getFirstImg = $GR->getArray('select file_route1 from '.$dbFIX.'pds_save where id = \''.$id.'\' and article_num = ' . $list['no']);
	$list['thumb'] = makeThumbImage($getFirstImg['file_route1'], $size);
